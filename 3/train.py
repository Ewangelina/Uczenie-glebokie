...
def train_bace(model, train_loader, val_loader, device, num_epochs=10, lr=0.001, patience=3):
...
# Training function for QM9 (Regression)
def train_qm9(model, train_loader, val_loader, device, num_epochs=10, lr=0.001, patience=10):
    model = model.to(device)
    optimizer = Adam(model.parameters(), lr=lr)
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_loader:
            ...
            optimizer.zero_grad()
            output = model(data.x, data.edge_index, data.batch).squeeze()
            loss = criterion(output, data.y.view(-1, 19))  # Ensure target dimension matches output
            loss.backward()
            optimizer.step()
            train_loss += loss.item() * data.num_graphs
        ...
        val_loss /= len(val_loader.dataset)
        val_mae = torch.mean(torch.abs(torch.tensor(val_preds) - torch.tensor(val_labels))).item()
