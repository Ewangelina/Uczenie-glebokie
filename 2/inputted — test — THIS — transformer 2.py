...
for seq_length in sl_tab: # Length of input sequence
    for hidden_size in hs_tab: # Size of LSTM hidden state
        memory = torch.rand(1, seq_length, hidden_size)
        tgt_mask = []
        ...
        for num_layers in nl_tab: # Number of LSTM layers
            for heads in hd_tab:
                ...
                tekst = "mała mi miała mały młyn i małego misia i małą miskę i "
                sequences = torch.tensor([med.encode(tekst)], dtype=torch.long)
                ...
                with torch.no_grad():
                    for j in range(54):
                        ...
                        letter = med.decode([round(float(outputs))])
                        print(letter)
                        tekst = tekst[1:] + str(letter)
