    ...
    for d_model_param in d_model_table: # Size of LSTM hidden state
      #with open(outfile, 'a') as f:
        #with redirect_stdout(f):
                # Data
                dataset = LetterDataset(seq_length, '.\\data\\test_data.txt')
                ...
                model_name = ".\\saved_models\\" + "best_" + str(d_model_param) + "_lightning_model.sav"
                model = pickle.load(open(model_name, 'rb')).to(device)
                ...
                if False:
                  trainer = L.Trainer(max_epochs=30, default_root_dir=".\\saved_models\\")
                ...
                if True:
                    ...
                    for sequences, targets in data_loader:
                        ...
                        predicted_id = torch.tensor([torch.argmax(predictions[-1,:])]) #last char id
                        if target.item() == predicted_id.item():
                            accuracy += 1
                ...
                ## Use model
                text = "mała mi miała mały młyn i małą miskę i małego misi"
                model_input = torch.tensor(med.encode(text), dtype=torch.long).to(device)
                predictions = model(model_input)
                o = ""
                for char in range(len(predictions)):
                    predicted_id = torch.tensor([torch.argmax(predictions[char,:])])
                    o = o + str(med.decode([predicted_id.item()]))
